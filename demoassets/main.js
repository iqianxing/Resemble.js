$(function () {
	...
	var socket = io();
	function sendCompareResult(result) {
		socket.emit('compare finish', result);
	}
	...
	(function () {
		...
		xhr.open('GET', 'screenshots/iPhone6/hello.png', true);
		...
		xhr2.open('GET', 'screenshots/nexus10/hello.png', true);
		...
		$('#example-images').click(function () {
			...
			$('#dropzone1').html('<img src="screenshots/iPhone6/hello.png"/>');
			$('#dropzone2').html('<img src="screenshots/nexus10/hello.png"/>');

			$.when(done, dtwo).done(function (file, file1) {
				if (typeof FileReader === 'undefined') {
					resembleControl = resemble('screenshots/iPhone6/hello.png').compareTo('screenshots/nexus10/hello.png').onComplete(onComplete);
				} else {
					resembleControl = resemble(file).compareTo(file1).onComplete(onComplete);
				}
			});
